...
module.exports = function Motor(x, y, parent) {
    ...
    //maybe this is a problematic motor, on reverse
    _this.reverse = false;
    ...
    _this.SPEED_TIME = 2.2 * 1000;
    _this.SLOW__TIME = 4.4 * 1000;
    ...
    _this.sendCommand = (cmd) => {
        // get back up from an admin command
        if (_this.command >= 0xFB) {
            _this.command = 0x14;
        }
        if (cmd <= 0xF0) {
            //we invert the command so the reverse motor goes on the right direction
            if (_this.reverse) {
                cmd = 0xF0 - cmd;
            }
            if (cmd != _this.command) {
                ...
                if (!_this.locked) {
                    ...
                    helper.preciseSetTimeout(_this.getSteps(cmd), _this.getFPS(), (count) => {
